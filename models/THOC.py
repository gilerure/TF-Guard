...
class THOC(nn.Module):
    ...
    def forward(self, X):
        ...
        for i, cluster in enumerate(self.clusters):
            # Assignment
            f_normed = F.normalize(f_t_bar, p=2, dim=-1, eps=1e-8)  # (B, P, H)
            c_normed = F.normalize(cluster, p=2, dim=0, eps=1e-8)   # (H, K)
            cosine_similarity = torch.einsum("Bph,hn->Bpn", f_normed, c_normed)

            cosine_similarity = torch.clamp(cosine_similarity, -10, 10) 
            P_ij = F.softmax(cosine_similarity / self.tau, dim=-1)
...
class Solver():
    def __init__(self, configs,
        ...
        self.optimizer = torch.optim.AdamW(params=self.model.parameters(), lr=self.lr, weight_decay=self.L2_reg)
    ...
    def fit(self, train_loader, valid_loader):
        for epoch in range(1, self.epochs + 1):
            ...
            for idx, (d, _) in loop:
                d = d.to(self.device)
                self.scheduler.step()
                anomaly_score, loss_dict = self.model(d)

                self.optimizer.zero_grad()
                loss = loss_dict["L_THOC"] + self.lambda_orth * loss_dict["L_orth"] + self.lambda_tss * loss_dict["L_TSS"]
                loss.backward()
                self.optimizer.step()
            ...
            if len(valid_loader) > 0:
                ...
                with torch.no_grad():
                    for idx, (d, _) in loop:
                        d = d.to(self.device)
                        anomaly_score, loss_dict = self.model(d)
                        loss = loss_dict["L_THOC"] + self.lambda_orth * loss_dict["L_orth"] + self.lambda_tss * \
                                loss_dict["L_TSS"]
