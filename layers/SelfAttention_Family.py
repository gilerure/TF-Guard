...
class CrossAttention(nn.Module):
    def __init__(self, in_channels, d_keys, d_values, scale=None):
        super(CrossAttention, self).__init__()
        self.scale = scale
        self.query_projection = nn.Linear(in_channels, d_keys)
        self.key_projection = nn.Linear(in_channels, d_keys)
        self.value_projection = nn.Linear(in_channels, d_values)
        self.out_projection = nn.Linear(d_values, in_channels)
...
class AnomalyAttentionLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
                 d_values=None):
        super(AnomalyAttentionLayer, self).__init__()
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        keys = self.key_projection(keys).view(B, S, H, -1)
        values = self.value_projection(values).view(B, S, H, -1)
        sigma = self.sigma_projection(original_queries).view(B, L, H)
